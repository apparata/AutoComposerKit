...
class BassGenerator: Generator {
    ...
    func applyNotes(channel: Int, pattern: Pattern, rhythm: [Int], beatBegin: Int, beatLength: Int, rootKey: Key, keyChord: Key) {
        ...
        for row in beatBegin..<beatEnd {
            if rhythm[row] & 1 > 0 {
                let note = (Float.random(in: 0...1) < 0.5) ? base - 12 : base
                pattern.data[row][channel] = [note, Instrument.bass.rawValue, 255, 0, 0]
                
                if leadIn != 0 && Float.random(in: 0...1) < 0.4 {
                    let gran = 2
                    var count = 1
                    
                    if leadIn > gran * 2 && Float.random(in: 0...1) < 0.4 {
                        count += 1
                        if leadIn > gran * 3 && Float.random(in: 0...1) < 0.4 {
                            count += 1
                        }
                    }
                    ...
                    for j in 0..<count {
                        pattern.data[row - (j + 1) * gran][channel] = [
                            (Float.random(in: 0...1) < 0.5) ? base + 12 : base,
                            Instrument.bass.rawValue,
                            0xFF,
                            19, // ord('S') - ord('A') + 1
                            0xC0 + [1, 2].randomElement()!
                        ]
                    }
                }
                ...
                if Float.random(in: 0...1) < 0.2 {
                    ...
                    if Float.random(in: 0...1) < 0.4 {
                        ...
                        pattern.data[row][channel][4] = 0xC0 + [1, 2].randomElement()!
                    } else {
